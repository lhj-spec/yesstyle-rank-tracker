...
.brand-dropdown { position: relative; display: inline-block; }
.brand-dropdown-btn { display: flex; align-items: center; gap: 6px; padding: 5px 12px; font-size: 13px; border: 1px solid #dfe6e9; border-radius: 8px; background: white; color: #2d3436; cursor: pointer; min-width: 200px; justify-content: space-between; white-space: nowrap; }
.brand-dropdown-btn:hover, .brand-dropdown-btn.open { border-color: #6c5ce7; box-shadow: 0 0 0 2px rgba(108,92,231,.15); }
.brand-dropdown-arrow { font-size: 10px; color: #b2bec3; margin-left: 4px; }
.brand-dropdown-panel { position: absolute; top: calc(100% + 4px); left: 0; z-index: 200; background: white; border: 1px solid #dfe6e9; border-radius: 8px; box-shadow: 0 4px 16px rgba(0,0,0,.12); min-width: 220px; display: none; flex-direction: column; overflow: hidden; }
.brand-dropdown-panel.open { display: flex; }
.brand-panel-search { border: none; border-bottom: 1px solid #f0f0f0; padding: 8px 12px; font-size: 12px; outline: none; width: 100%; box-sizing: border-box; }
.brand-checkbox-list { overflow-y: auto; max-height: 240px; padding: 4px 0; }
.brand-checkbox-item { display: flex; align-items: center; gap: 8px; padding: 6px 12px; cursor: pointer; font-size: 13px; color: #2d3436; user-select: none; }
.brand-checkbox-item:hover { background: #f5f6fa; }
.brand-checkbox-item input[type="checkbox"] { accent-color: #6c5ce7; width: 14px; height: 14px; cursor: pointer; flex-shrink: 0; }
.brand-checkbox-item.checked { color: #6c5ce7; font-weight: 600; }
  ...
  <div class="filter-bar">
    ...
    <div class="filter-row">
      ...
      <div class="brand-dropdown" id="brandDropdown">
        <button type="button" class="brand-dropdown-btn" id="brandDropdownBtn">
          <span id="brandDropdownLabel">JUMISO</span>
          <span class="brand-dropdown-arrow">▾</span>
        </button>
        <div class="brand-dropdown-panel" id="brandDropdownPanel">
          <input type="text" class="brand-panel-search" id="brandPanelSearch">
          <div class="brand-checkbox-list" id="brandCheckboxList"></div>
        </div>
      </div>
    </div>
  </div>
...
function applyLang() {
  ...
  // 브랜드 드롭다운 버튼 텍스트 언어 처리
  if (typeof _updateBrandBtn === 'function') _updateBrandBtn();
  // statBrands 기본값 처리
...
// ── 브랜드 체크박스 드롭다운 초기화 ─────────────────────────
let _updateBrandBtn, _buildBrandList;

function initBrandTags() {
  const SORTED = [...ALL_BRANDS].sort((a, b) => a.localeCompare(b, undefined, { sensitivity: 'base' }));
  const btn    = document.getElementById('brandDropdownBtn');
  const panel  = document.getElementById('brandDropdownPanel');
  const label  = document.getElementById('brandDropdownLabel');
  const search = document.getElementById('brandPanelSearch');
  const list   = document.getElementById('brandCheckboxList');

  _updateBrandBtn = function() {
    if (state.selectedBrands.length === 0) {
      label.textContent = lang === 'ko' ? '전체 브랜드' : 'All Brands';
    } else if (state.selectedBrands.length === 1) {
      label.textContent = state.selectedBrands[0];
    } else {
      label.textContent = \`\${state.selectedBrands.length}\${lang === 'ko' ? '개 브랜드' : ' brands'}\`;
    }
  };

  _buildBrandList = function(filter) {
    const q = (filter || '').toLowerCase();
    list.innerHTML = SORTED
      .filter(b => !q || b.toLowerCase().includes(q))
      .map(b => {
        const on = state.selectedBrands.includes(b);
        return \`<label class="brand-checkbox-item\${on ? ' checked' : ''}"><input type="checkbox" value="\${b}"\${on ? ' checked' : ''}>\${b}</label>\`;
      }).join('');
  };
  ...
  btn.addEventListener('click', () => {
    ...
    if (opening) { search.value = ''; _buildBrandList(''); search.focus(); }
  });

  document.addEventListener('click', e => {
    if (!document.getElementById('brandDropdown').contains(e.target)) {
      panel.classList.remove('open');
      btn.classList.remove('open');
    }
  });

  search.addEventListener('input', () => _buildBrandList(search.value));

  list.addEventListener('change', e => {
    if (e.target.type !== 'checkbox') return;
    const brand = e.target.value;
    const item  = e.target.closest('.brand-checkbox-item');
    if (e.target.checked) {
...
function initResetBtn() {
  document.getElementById('filterReset').addEventListener('click', () => {
    ...
    document.getElementById('brandPanelSearch').value = '';
    _buildBrandList('');
    _updateBrandBtn();
    render();
  });
